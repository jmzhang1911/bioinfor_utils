...
class MyRunner:
    ...
    # @classmethod
    # def cmd_wrapper(cls, cmd_name='qsub_cmd', threads_num=8, qsub=False, queue='low.q', test=False, max_sample=False):
    #     def _logger(fn):
    #         @functools.wraps(fn)
    #         def _wrapper(*args, **kwargs):
    #             cmd_list = fn(*args, **kwargs)[0]
    #             MyPath.mkdir('__work_sh')
    #             with open('__work_sh/' + cmd_name + '.sh', 'w') as f:
    #                 for cmd in cmd_list:
    #                     f.write(cmd + '\n')
    #
    #             cls.runner(cmd_list,
    ...
    #
    # @staticmethod
    # def runner(cmd_list, cmd_name='qsub_cmd', threads_num=8, qsub=False, queue='low.q', test=False, max_sample=False):
    #     run_qsub = '/share/nas2/genome/bmksoft/tool/qsub_sge_plus/v1.0/qsub_sge.plus.sh'
    #
    #     def _runner(cmd):
    #         cmd, qsub_name = cmd
    ...
    #
    #         else:
    #             MyPath.mkdir('./__qsub_cmds')
    #             cmd_file = './__qsub_cmds/' + qsub_name + '.sh'
    #             with open(cmd_file, 'w') as f:
    #                 f.write(cmd)
    #             cmd = 'bash {} --reqsub --independent {} --queue {}'.format(run_qsub, cmd_file, queue)
    #
    #         logging.info('subprocess running cmd: {}'.format(cmd))
    #
    #         if test:
    #             logging.info(cmd)
    #             time.sleep(3)
    ...
    #             p.wait()
    #             logging.info('subprocess cmd done: {}'.format(cmd))
    #
    #     if not cmd_list:
    #         logging.info('cmds is None')
    #         return None
    #
    #     if isinstance(cmd_list, tuple):
    #         # get the first arg if the function of cmds returns a (cmds, *args)
    #         cmd_list = cmd_list[0]
    #
    #     if len(cmd_list) == 0:
    #         raise Exception('there is no cmds.Please check your function which creating the cmds')
    #
    #     if not isinstance(cmd_list[0], tuple):
    #         # [(cmd1,'cmd1_name'), (cmd2, 'cmd2_name')]
    #         cmd_list = [(cmd, cmd_name + '_' + str(seq_name)) for seq_name, cmd in enumerate(cmd_list)]
    #
    #     if max_sample:
    #         threads_num = len(cmd_list)
    #
    #     with ThreadPoolExecutor(max_workers=threads_num) as executor_cmd:
    #         executor_cmd.map(_runner, cmd_list)
    ...
    def _run_cmd(func_name, cmd_list):
        ...
        num = 1
        while Path(cmd_file).exists():
            cmd_file = 'cmds_sh/{}_{}.sh'.format(func_name, num)
            num += 1
