...
spec <- matrix(c(
  'rds'        , 'R', 2, 'character', 'Rdata after step1 filter, example: upload.Rdata',
...
samples.As <- strsplit(unlist(strsplit(group, split = "_vs_")), split = "_")[[1]]
samples.Bs <- strsplit(unlist(strsplit(group, split = "_vs_")), split = "_")[[2]]
samples.A <- unlist(strsplit(samples.As, split = "_"))
samples.B <- unlist(strsplit(samples.Bs, split = "_"))
all.samples <- c(samples.A, samples.B)
...
ExpHeatmapAndDotplot <- function(seurat.object, diff.exp, odir, cluster){
  ...
  exp_data <- GetAssayData(seurat.object, assay = my.assay, slot = "data")
  exp_data <- as.matrix(exp_data[intersect(diff.exp$gene, rownames(exp_data)), data$cell])
  exp_data <- log10(exp_data+1)
  top_anno <- HeatmapAnnotation(
  cluster = anno_block(gp = gpar(fill = rainbow(length(levels(sample_info))), col = "white"), # 设置填充�?
...
TopNPlot <- function(seurat.data, top.gene, odir, top = 3, x){
  ...
  top_gene_num = length(top.gene)
  if(top_gene_num < 5){tmp_ncol = top_gene_num}else{tmp_ncol = ceiling(top_gene_num / 2)}
  
  p.tsne <- FeaturePlot(seurat.data, features = top.gene, reduction = "tsne", slot = "data", cols = c("lightgrey", "red"), ncol = tmp_ncol, pt.size = 0.1) & NoLegend() & GlobPlotTheme()
  outfile <- paste(paste("cluster", x, sep = ""), paste("top", top, sep = ""), "markerTsne", sep = "_")
  SavePlot(od = odir, filename = outfile, data = p.tsne)
  p.umap <- FeaturePlot(single.integrated, features = top.gene, reduction = "umap", slot = "data",, cols = c("lightgrey", "red"), ncol = tmp_ncol, pt.size = 0.1) & NoLegend() & GlobPlotTheme()
...
all.cells <- single.integrated@meta.data %>% filter(sample %in% all.samples) %>% .[, "cell", drop = TRUE]
single.integrated <- single.integrated[,all.cells]
print(unique(single.integrated$sample))
...
for( x in 0:(length(unique(Idents(single.integrated)))-1)){
  ...
  #2021.12.16�����ļ�Ϊcontrol vs case�˴���Ҫ�޸�˳��
  group.B <- single.integrated@meta.data %>% filter(seurat_clusters == x & sample %in% samples.A)
  group.A <- single.integrated@meta.data %>% filter(seurat_clusters == x & sample %in% samples.B)
  ...
  if (nrow(group.A) <= 2 | nrow(group.B) <= 2 ) {
	...
	diff.all <- FindMarkers(object = single.integrated, assay = my.assay, slot = "data", ident.1 = group.A$cell, ident.2 = group.B$cell, min.pct = 0.25, logfc.threshold = 0) %>% rownames_to_column("gene") %>% rename(avg_logFC = avg_log2FC) %>% .[order(.[,'avg_logFC'], decreasing = TRUE),]
	print("FindMarkers done...")
	print("diff start...")
	
	# 2021.12.15
	# diff.all$avg_logFC <- diff.all$avg_logFC/log(2) 
	#colnames(diff.all)[2] <- 'avg_logFC'
	
	diff.all <- AddId(diff.all, ensemble.id.symbol)
        header <- c("ID", "symbol", "Pvalue", "log2FC", "pct.1", "pct.2", "Qvalue")
        outfile <- paste(group, paste("cluster", x, sep = ""), "all_featuregene.xls", sep = ".")
        write.table(t(header), file = file.path(outputdir, outfile), sep = "\t", row.names = FALSE, col.names = FALSE, quote = FALSE)
        write.table(diff.all, file = file.path(outputdir, outfile), sep = "\t", row.names = FALSE, col.names = FALSE, quote = FALSE, append = T)
	print("Add symbol done")
        if(is.null(FDR) && is.null(pvalue)){
          if(!is.null(fold)){
            diff.filter <- diff.all %>% filter(abs(avg_logFC) > logfc.threshold)
          }else{
	...
	if(!is.null(FDR)){
  	  if(!is.null(fold)){
            diff.filter <- diff.all %>% filter(p_val_adj < FDR & abs(avg_logFC) > logfc.threshold)
          }else{
        ...
        if(!is.null(pvalue)){
          if(!is.null(fold)){
            diff.filter <- diff.all %>% filter(p_val < pvalue & abs(avg_logFC) > logfc.threshold)
          }else{
	...
	diff.features <- diff.filter[diff.filter$gene %in% intersect(rownames(single.integrated), diff.filter$gene),]
	if(nrow(diff.features) < 2){
		diffList[i] <- 1
	          next
	}
	if (nrow(diff.filter) < top.n){
	  ...
	  topmarker <- diff.features %>% top_n(., n = top, wt = avg_logFC) %>% .[order(.[,'avg_logFC'], decreasing = TRUE),] %>%  .[, "gene", drop = TRUE] %>% head(n = top)  %>% unique()
