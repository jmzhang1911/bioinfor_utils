...
PrintUsage <- function(){
  cat("ProgramName:TCR.R
Version: Version v1.0
Contact: zhengly <zhengly@biomarker.com.cn> 
Program Date:   2020.12.15 
Description: this program is used to TCR ;\n")
...
##V/J Gene
vizGene <- function(immdata=immdata ,Gene="TRAV"){
  ##Gene "TRAV" "TRBV" "TRAJ" "TRBJ"
  if(str_sub(Gene,4,4)=="V"){
    gene <- geneUsage(immdata[["data"]], "hs.trav")
  }else if(str_sub(Gene,4,4)=="J"){
    gene <- geneUsage(immdata[["data"]], "hs.traj")
  }
  ...
  p <- trav %>% ggplot() + geom_col(aes(x=Names,y=Count,fill=Sample),
                               position =position_dodge2(padding = 0, preserve = "single"))+
    theme_bw()+theme(panel.grid =element_blank())+scale_fill_npg()+
    labs(x=paste(str_sub(Gene,1,3),str_sub(Gene,4,4),"Gene",collapse = ""),y="Count",fill = "Sample",title = "")+
...
if(type=="TRA"){
  TRAV=vizGene(immdata,"TRAV")
  SavePlot(od,paste0(type,"_TRAV_gene"),TRAV,width = 12,height = 8)
  TRAJ=vizGene(immdata,"TRAJ")
  SavePlot(od,paste0(type,"_TRAJ_gene"),TRAJ,width = 12,height = 8)
}else if(type=="TRB"){
  TRBV=vizGene(immdata,"TRBV")
  SavePlot(od,paste0(type,"_TRBV_gene"),TRBV,width = 12,height = 8)
  TRBJ=vizGene(immdata,"TRBJ")
  SavePlot(od,paste0(type,"_TRBJ_gene"),TRBJ,width = 12,height = 8)
}
print("V/J Gene done..")
...
###V/J in CDR3
for(i in 1:length(immdata$meta$Sample)){
    spect_v <- spectratype(immdata$data[[i]], .quant = "count", .col = "aa+v") %>% vis()
    SavePlot(od,paste0(type,"_CDR3_V_",immdata$meta$Sample[i]),spect_v,width = 12,height = 8)
    spect_j <- spectratype(immdata$data[[i]], .quant = "count", .col = "aa+j") %>% vis()
    SavePlot(od,paste0(type,"_CDR3_J_",immdata$meta$Sample[i]),spect_j,width = 12,height = 8)
}
print("V/J CDR3 done..")
