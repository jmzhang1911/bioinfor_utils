...
'Description: BCR analysis
Version: Version v1.0
Contact: zhengly <zhengly@biomarker.com.cn> 
Program Date:   2020.12.20 %o',
...
open(SH1,">$od/work_sh/BCR_analysis.sh") or die $!;
my $cmd="/share/nas2/genome/biosoft/R/3.6.1/bin/Rscript $Bin/BCR.R -i $od/input -o $od/IGH -t IGH -s $config{'Species'}\n";

$cmd.="/share/nas2/genome/biosoft/R/3.6.1/bin/Rscript $Bin/BCR.R -i $od/input -o $od/IGK -t IGK -s $config{'Species'}\n";

$cmd.="/share/nas2/genome/biosoft/R/3.6.1/bin/Rscript $Bin/BCR.R -i $od/input -o $od/IGL -t IGL -s $config{'Species'}\n";

$cmd.="/share/nas2/genome/biosoft/R/3.6.1/bin/Rscript $Bin/Stat.R -i $od/input -o $od -t BCR \n";

if(-e "$od/../../SC_analysis/step3_integrated/integrated/single_analysis.Rda"){
  $cmd.="export PATH=/share/nas2/genome/biosoft/gcc/5.5.0/bin/:\$PATH && export LD_LIBRARY_PATH=/share/nas2/genome/biosoft/gcc/5.5.0/lib:/share/nas2/genome/biosoft/gcc/5.5.0/lib64:/share/nas2/genome/cloud_soft/developer_platform/module_script/module_immune_repertoire/trunk/gsl/2.2.1/lib/:\$LD_LIBRARY_PATH && /share/nas2/genome/biosoft/R/4.0.3/bin/Rscript $Bin/scRepertoire.R -i $od/input -o $od/combined -r $od/../../SC_analysis/step3_integrated/integrated/single_analysis.Rda -t B";
}else{
  my @rda=glob("$od/../../SC_analysis/step2_analysis/*/seurat/single_analysis.Rda"); 
  my $rda=join("",@rda);
  if(-e "$rda"){
    $cmd.="export PATH=/share/nas2/genome/biosoft/gcc/5.5.0/bin/:\$PATH && export LD_LIBRARY_PATH=/share/nas2/genome/biosoft/gcc/5.5.0/lib:/share/nas2/genome/biosoft/gcc/5.5.0/lib64:/share/nas2/genome/cloud_soft/developer_platform/module_script/module_immune_repertoire/trunk/gsl/2.2.1/lib/:\$LD_LIBRARY_PATH && /share/nas2/genome/biosoft/R/4.0.3/bin/Rscript $Bin/scRepertoire.R -i $od/input -o $od/combined -r $rda -t B";
  }
}
print SH1 "$cmd";
close SH1;

&qsub("$od/work_sh/BCR_analysis.sh");
&qsubCheck("$od/work_sh/BCR_analysis.sh");
