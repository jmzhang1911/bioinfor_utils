...
def addTable(tmp,isfirst):
    ...
    if  isfirst:
        for i in tmp:
            result+="<th nowrap>%s</th>"%i
    else:
        for i in tmp:
            j = get_number_from_str(i)
            result += "<td nowrap>%s</td>" % j
    return result
def get_number_from_str(tmp):
    result=""
    ## int
    p1 = re.compile(r'^[-+]?[1-9][0-9]*$')
    ## float
    p2 = re.compile(r'^[-+]?[0-9]+\.?[0-9]+$')
    ## int(float%?)
    p3 = re.compile(r'^[-+]?[0-9]+\.?[0-9]+\([-+]?[0-9]+\.?[0-9]+%?\)$')
    r1 = p1.match(tmp)
    r2 = p2.match(tmp)
    r3 = p3.match(tmp)
    if r1:
        result = format(int(tmp),',')
    elif r2:
        t2 = ("%.2f" % float(tmp))
        result = format(float(t2),',')
    elif r3:
        p4 = re.compile(r'[-+]?[0-9]+\.?[0-9]+')
        [tmp3,tmp4] = p4.findall(tmp)
        t3 = format(int(tmp3),',')
        t4_1 = ("%.2f" % float(tmp4))
        t4_2 = format(float(t4_1),',')
        result = tmp.replace(tmp3,t3)
        result = result.replace(tmp4,t4_2)
    else :
        result = tmp
    return result
def timenow():
...
parser.add_argument('-l', '--tableLineNum', dest='tableLineNum', required=False,type=int,default=60, help='Input max table line numbers to show, default:60')
...
divFooter<<p(u'公司地址：北京市顺义区南法信镇府前街12号顺捷大厦6层'.encode("UTF-8"))
