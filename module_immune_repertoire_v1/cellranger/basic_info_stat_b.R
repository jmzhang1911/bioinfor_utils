...
header3 <- matrix(c("sampleID", "Reads Mapped to Any V(D)J Gene", "Reads Mapped to IGH","Reads Mapped to IGK","Reads Mapped to IGL","Median IGH UMIs per Cell","Median IGK UMIs per Cell","Median IGL UMIs per Cell","Cells With Productive V-J Spanning Pair","Cells With IGH Contig","Cells With IGK Contig","Cells With IGL Contig","Cells With CDR3-annotated IGH Contig","Cells With CDR3-annotated IGK Contig","Cells With CDR3-annotated IGL Contig","Cells With V-J Spanning IGH Contig","Cells With V-J Spanning IGK Contig","Cells With V-J Spanning IGL Contig"), nrow = 1)
write.table(header1, file = paste(opt$od, "b_total_seqence_info_stat.xls", sep = "/"), sep = "\t", quote = F, row.names = F, col.names = F)
write.table(header2, file = paste(opt$od, "b_total_cell_info_stat.xls", sep = "/"), sep = "\t", quote = F, row.names = F,col.names = F)
write.table(header3, file = paste(opt$od, "b_total_vdj_info_stat.xls", sep = "/"), sep = "\t", quote = F, row.names = F,col.names = F)
sapply(files, function(x){
	...
	write.table(seq.data, file = paste(opt$od, "b_total_seqence_info_stat.xls", sep = "/"), sep = "\t", quote = F, row.names = F, col.names = F, append = TRUE)
	write.table(cell.data, file = paste(opt$od, "b_total_cell_info_stat.xls", sep = "/"), sep = "\t", quote = F, row.names = F, col.names = F, append = TRUE)
	write.table(mapped.data, file = paste(opt$od, "b_total_vdj_info_stat.xls", sep = "/"), sep = "\t", quote = F, row.names = F, col.names = F, append = TRUE)
})
