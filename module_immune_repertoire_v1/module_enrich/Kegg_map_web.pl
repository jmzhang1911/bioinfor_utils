...
my %file_db; 
$file_db{kegg_png_file}="/share/nas2/database/KEGG/201703/kegg_png/";
...
#        print OUT $header;
	...
	print "draw_KEGG_histogram.pl --ipf $kegg_out/$opts{k}.KEGG.xls --opd $kegg_out --prf $opts{k}.KEGG\n";
	&runOrDie("draw_KEGG_histogram.pl --ipf $kegg_out/$opts{k}.KEGG.xls --opd $kegg_out --prf $opts{k}.KEGG");
	my %ko;
	...
	while (<UD>) {
		...
		if($opts{target} == 0){
			...
			$ko{$tmp[0]}{color}="target";	
			$ko{$tmp[0]}{value}.=",$tmp[1]";
		}
		$ko{$tmp[0]}{value}=~s/^,//;
	}
	close (UD);
	...
	while (<LI>) {
		...
		for my $i (0..$#tmp_2) {
			...
			$png_new{$tmp_1[1]}{$tmp_3[$i]}{COLOR} = "target" if ($png_new{$tmp_1[1]}{$tmp_3[$i]}{color} =~/target/);
		}
	}
	%ko=();
	close (LI);
...
#			print "@K_ID\n",'=' x 10,"\n"; #tag
		...
		if($type=~/rect/){
			...
			for my $x($p1+1..$p3-1){
				for my $y ($p2+1..$p4-1){
					...
					$im->fillToBorder($x,$y,$black,$red)	if($r==255 and $g ==255 and $b==255);
				}
			}
	...
	if ($tar != 0) {
                if($type=~/rect/){
                        ...
                        $poly->addPt($p1,$p2);$poly->addPt($p1,$p4);$poly->addPt($p3,$p4);$poly->addPt($p3,$p2);
                        $im->openPolygon($poly,$black);
                        $im->fillToBorder($p1+1,$p2+1,$black,$gray);
                        for my $x($p1+1..$p3-1){
                                for my $y ($p2+1..$p4-1){
                                        my $colorIndex=$im->getPixel($x,$y);
                                        my ($r,$g,$b)=$im->rgb($colorIndex);
                                        $im->fillToBorder($x,$y,$black,$gray)    if($r==255 and $g ==255 and $b==255);
                                }
                        }
                        $html{$koxx}{"$p1,$p2,$p1,$p4,$p3,$p4,$p3,$p2"}{updown}="Target";
                        $html{$koxx}{"$p1,$p2,$p1,$p4,$p3,$p4,$p3,$p2"}{color}="FFA500";
                }elsif($type=~/line/){
                        my @temp=split/\t/,$rect;
                        for(my $i=1;$i<=$#temp-1;$i++){
                                my ($p1,$p2,$p3,$p4) = $temp[$i] =~ /\((\d+),(\d+),(\d+),(\d+)\)/;
                                ...
                                $im->openPolygon($poly,$gray);
                                $html{$koxx}{"$p1,$p2,$p3,$p4"}{updown}="Target";
                                $html{$koxx}{"$p1,$p2,$p3,$p4"}{color}="FFA500";
                        }
                }
        }
		...
		if($type=~/rect/){
			...
			for my $x($p1+1..$p3-1){                    ##look for the close area and fill color
				for my $y ($p2+1..$p4-1){
					my $colorIndex=$im->getPixel($x,$y);
					my ($r,$g,$b)=$im->rgb($colorIndex);
					$im->fillToBorder($x,$y,$black,$green) if($r==255 and $g ==255 and $b==255);
				}
			}
...
____________HTML
			foreach my $coords ( keys %{$html{$koxx}} ) {
				...
				if ($updown eq 'Up' || $updown eq "Down") {
					print HTML <<____________HTML;
...
____________HTML
				}elsif($updown eq "Target"){
					print HTML <<____________HTML;
<area shape='poly' coords='$html{$koxx}{$coords}{html_coord}' href='http://www.kegg.jp/dbget-bin/www_bget?$html{$koxx}{$coords}{KO}' target="_blank" onmouseover='javascript: showInfo("<ul><li style=\\"color: #$color;\\">$updown $info</li></ul>");' />
____________HTML
				}else{
...
sub help {#usage
	...
	Usage:
		...
		-i    <str>      input Result dir
		-o    <str>      output dir
 		-v               [option]   view process on screen
		-target int	 default 0 , if DEG lncRNA Trans this should be 1	
	Example��
