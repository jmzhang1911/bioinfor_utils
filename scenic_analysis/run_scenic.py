...
class PyScenic:
    ...
    RUN_down_analysis = _ / 'down_stream_analysis.R'
    pp = _ / 'pp.R'

    def __init__(self, seurat_obj,
                 species, cell_type='cellType',
                 output='pySCENIC_results', threads=20,
                 groups='orig.ident'):
        self.output = output
    ...
    def run_ctx_aucell(self):
        ...
        self.sample_scenic = self.output + '/sample_SCENIC.loom'
        if Path(self.sample_scenic).exists():
            logging.info('aucell analysis has benn done, skipping ...')
        else:
            logging.info('doing aucell analysis ...')
            cmd = '{} {} aucell {} {} --output {} --num_workers {}' \
                .format(self.PYTHON,
                        self.RUN_pyscenic,
                        self.loom,
                        reg,
    ...
    @MyRunner.count_running_time
    @MyRunner.cmd_wrapper(threads_num=1)
    def run_down_analysis(self):
        logging.info('running down_analysis ...')
        cmd = '{} {} --meta_data {} --output {} --cell_type {} --groups {} --sample_loom {}'. \
            format(self.RSCRIPT, self.RUN_down_analysis,
                   self.meta_data,
                   self.output,
                   self.cell_type,
                   self.groups,
